"""Conversation-scoped browser workspace routes.

Routes that load a conversation, interact with the browser worker, record
timeline events, and persist workspace state (annotations, element maps,
cooperation snapshots).

Late-binding via ``import sessions as _sessions`` ensures test
monkeypatches on ``sessions.get_container`` and shared helpers are
...
def register_browser_workspace_routes(router) -> None:
    """Register conversation-scoped browser workspace endpoints on the sessions router."""
...
# ---------------------------------------------------------------------------
# WebSocket helper (used only by the WS endpoint above)
# ---------------------------------------------------------------------------
