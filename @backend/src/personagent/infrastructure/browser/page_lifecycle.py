...
class BrowserPageLifecycle:
    ...
    async def open(
        ...
        *,
        ...
        if target_url is None and result_index is not None:
            target_url, matched_search_id = self._w._result_url(
                conversation_id,
                session,
                result_index,
                search_id=search_id,
            )
            matched_search_title = self._w._result_title(
                conversation_id,
                result_index,
                search_id=matched_search_id or search_id,
            )
        elif target_url and search_id:
            matched_search_id = self._w._match_search_result_url(
                conversation_id,
                target_url,
                search_id=search_id,
            )
            if matched_search_id:
                matched_search_title = self._w._match_search_result_title(
                    conversation_id,
                    target_url,
        ...
        existing_opened_page = self._w._opened_page_by_url(conversation_id, target_url)
        existing_page = (
            session.pages.get(existing_opened_page.page_id)
            if existing_opened_page is not None
            else None
        )
        if existing_opened_page is not None and existing_page is not None and self._w._page_is_open(existing_page):
            ...
            self._w._remember_current_url(conversation_id, existing_opened_page.final_url)
            self._w._attach_page_console_listeners(conversation_id, existing_opened_page.page_id, existing_page)
            session.touch()
            return self._w._browser_open_response(
        ...
        page = await self._w._new_session_page(session)
        ...
        try:
            ...
            await self._w._raise_if_search_blocked(page)
        except Exception:
            if close_failed_page:
                await self._w._best_effort_resource_call("browser_open_failed_page_close", page.close)
            raise
        ...
        self._w._remember_current_url(conversation_id, final_url)
        opened_page, reused_existing_page = self._w._cache_opened_page(
            conversation_id=conversation_id,
            url=target_url,
            final_url=final_url,
            title=title,
        ...
        if previous_page is not None and previous_page is not page and self._w._page_is_open(previous_page):
            await self._w._best_effort_resource_call("browser_reused_previous_page_close", previous_page.close)
        ...
        self._w._attach_page_console_listeners(conversation_id, opened_page.page_id, page)
        await self._w._cleanup_live_pages(conversation_id, session, keep_page_id=opened_page.page_id)
        session.touch()
        return self._w._browser_open_response(
            conversation_id=conversation_id,
    ...
    async def list_tabs(
        ...
        *,
        ...
        await self._w._cleanup_sessions()
        ...
        if session is None and not current_url and not pages:
            panel_tabs = await self._w._panel_session_tabs(max_tabs=max_tabs, exclude_conversation_id=conversation_id)
            if panel_tabs:
        ...
        if len(tabs) < max_tabs:
            ...
            panel_tabs = await self._w._panel_session_tabs(
                max_tabs=max_tabs,
    ...
    async def close_tab(
        ...
        *,
        ...
        if live_page is None and self._w._is_session_page_alias(conversation_id, session, target_page_id):
            live_page = self._w._preferred_session_page(session)
        closed = False
        if live_page is not None:
            await self._w._best_effort_resource_call("browser_control_close_page", live_page.close)
            closed = True
