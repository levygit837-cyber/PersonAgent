...
class KimiCodingAdapter(LLMBackendRepository):  # type: ignore[misc]
    ...
    def _parse_content_blocks(self, blocks: list[Any]) -> dict[str, Any]:
        ...
        for raw_block in blocks:
            ...
            if block_type == "text":
                ...
                tool_calls.append(self._tool_call_from_anthropic_block(raw_block))

        history_blocks = self._anthropic_history_blocks(
            {index: block for index, block in enumerate(blocks) if isinstance(block, dict)}
        )
        self._attach_anthropic_history_blocks(tool_calls, history_blocks)
    ...
    def _assistant_blocks(self, message: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        replay_blocks = self._anthropic_history_blocks_from_tool_calls(tool_calls)
        if replay_blocks:
    ...
    def _tool_calls_from_stream_state(
        ...
        history_blocks = self._anthropic_history_blocks(state.content_blocks)
        tool_calls = [
            self._tool_call_from_anthropic_block(block)
    ...
    def _anthropic_history_blocks(
        self,
        blocks: dict[int, dict[str, Any]],
    ) -> list[dict[str, Any]]:
        history: list[dict[str, Any]] = []
        for _, block in sorted(blocks.items()):
            normalized = self._normalize_anthropic_history_block(block)
    ...
    def _normalize_anthropic_history_block(
        ...
        if block_type == "tool_use":
            ...
            return {
                ...
                "id": tool_id,
                "name": name,
                "input": self._anthropic_tool_input(block),
            }

        return None

    def _attach_anthropic_history_blocks(
        self,
        tool_calls: list[dict[str, Any]],
    ...
    def _anthropic_history_blocks_from_tool_calls(
        self,
        tool_calls: list[dict[str, Any]],
        ...
        for tool_call in tool_calls:
            ...
            if blocks:
                return self._remap_anthropic_tool_ids(blocks, tool_calls)
        return None
            ...
            if next_block.get("type") == "tool_use":
                ...
                if new_id:
                    ...
                    if tool_call:
                        ...
                        next_block["input"] = self._parse_tool_arguments(
    ...
    def _tool_call_from_anthropic_block(self, block: dict[str, Any]) -> dict[str, Any]:
        raw_input = self._anthropic_tool_input(block)
        arguments = (
            raw_input
    ...
    def _anthropic_tool_input(self, block: dict[str, Any]) -> Any:
