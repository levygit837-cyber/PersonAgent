"""Investigation state and coverage tracking for the chat completion pipeline.

Extracted from messaging/state.py to keep the messaging layer focused on
message-formatting concerns.
"""
...
class InvestigationState:
    ...
    def classify(cls, request: ChatRequestDTO) -> InvestigationState:
        """Default classification: active when tools are enabled, depth from request.

        The model may reclassify depth during the first interaction if it
        judges the user's intent differently.  This keeps the gate lightweight
        and avoids brittle regex heuristics.
        """
