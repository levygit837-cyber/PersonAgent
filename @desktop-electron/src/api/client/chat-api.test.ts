...
describe("listModels", () => {
  it("returns normalized models from a flat array", async () => {
    ...
    const models = await listModels("http://localhost:8000", "kimi");

    expect(models).toHaveLength(1);
    expect(models[0]).toMatchObject({ id: "m1", name: "Model 1", provider: "kimi" });
  ...
  it("unwraps nested data field when present", async () => {
    ...
    const models = await listModels("http://localhost:8000", "codex");
  ...
  it("filters by capability when provided", async () => {
    ...
    await listModels("http://localhost:8000", "kimi", "vision");
...
describe("streamTeamChat", () => {
  it("yields parsed events from the team websocket", async () => {
    ...
    (mockSocket.onmessage as (ev: MessageEvent) => void)?.({ data: JSON.stringify({ event: "chunk", content: "hello" }) } as MessageEvent);
